...
# module load FIXTHIS/bcftools/1.16, plink/1.9
mem=16000
...
samples_final="$work_dir/sample_lists/samples.$tag.final" # this files includes IDs with underscores (GNH-related)
...
if [ ! -f $wes_new_prefix.vcf.gz ]; then
    ...
    filter --exclude 'MAF==0.0 | MAX(STRLEN(ALT))>25 | MAX(STRLEN(REF))>25' -Oz -o $wes_new_prefix.vcf.gz
...
bcftools query -f '%CHROM:%POS\n' $gsa_new_prefix.vcf.gz > $gsa_new_prefix.snpid
